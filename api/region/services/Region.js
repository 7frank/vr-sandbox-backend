...
module.exports = {
  ...
  fetch: (params) => {
    ...
    return Region
      .findOne(_.pick(params, _.keys(Region.schema.paths)))
      .populate(populate);
  },

  /**
   * Promise to count regions.
   *
   * @return {Promise}
   */
  ...
  add: async (values) => {
    ...
    const entry = await Region.create(data);
  ...
  edit: async (params, values) => {
    // Extract values related to relational data.
    const relations = _.pick(values, Region.associations.map(a => a.alias));
    const data = _.omit(values, Region.associations.map(a => a.alias));

    // Update entry with no-relational data.
    const entry = await Region.update(params, data, { multi: true });
  ...
  remove: async params => {
    ...
    await Promise.all(
      Region.associations.map(async association => {
